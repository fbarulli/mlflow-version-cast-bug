...
class SqlAlchemyStore(AbstractStore):
    ...
    def _get_or_create_engine(cls, db_uri: str) -> sqlalchemy.engine.Engine:
        ...
        if db_uri not in cls._engine_map:
            with cls._engine_map_lock:
                if db_uri not in cls._engine_map:
                    cls._engine_map[db_uri] = create_sqlalchemy_engine_with_retry(
                        db_uri)
    ...
    def __init__(self, db_uri, read_db_uri=None):
        ...
        if read_db_uri and read_db_uri != db_uri:
            ...
            ReadSessionMaker = sqlalchemy.orm.sessionmaker(
    ...
    def _get_query(self, session, model):
        ...
        if model in _WORKSPACE_MODELS:
            query = query.filter(
                model.workspace == self._get_active_workspace())
        return query
    ...
    def _get_eager_registered_model_query_options():
        ...
        return [
            sqlalchemy.orm.subqueryload(
                SqlRegisteredModel.registered_model_tags),
            sqlalchemy.orm.subqueryload(
                SqlRegisteredModel.registered_model_aliases),
        ]
    ...
    def _get_latest_versions_for_models(
        ...
        result: dict[str, list[SqlModelVersion]] = {
        ...
        with self.ManagedSessionMaker(read_only=False) as session:
            try:
                ...
                handle_resource_already_exist_error(
                    name, has_prompt_tag(
                        existing_model._tags), has_prompt_tag(tags)
                )
    ...
    def _get_registered_model(self, session, name, eager=False):
        ...
        if eager:
            query = query.options(
                *self._get_eager_registered_model_query_options())
    ...
    def rename_registered_model(self, name, new_name):
        ...
        with self.ManagedSessionMaker(read_only=False) as session:
            ...
            try:
                ...
                sql_registered_model.last_updated_time = updated_time
                session.add_all([sql_registered_model] +
    ...
    def search_registered_models(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            next_page_token = self._compute_next_token(
                max_results_for_query, len(
                    sql_registered_models), offset, max_results
            )

            # Batch-fetch latest versions for all models to avoid N+1 queries
            model_names = [
                rm.name for rm in sql_registered_models[:max_results]]
            latest_versions_map = self._get_latest_versions_for_models(
                session, model_names)

            rm_entities = [
                rm.to_mlflow_entity(
                    preloaded_latest_versions=latest_versions_map.get(rm.name))
    ...
    def _get_search_registered_model_filter_query(self, session, parsed_filters, dialect):
        ...
        for f in parsed_filters:
            ...
            if type_ == "attribute":
                ...
                attr_filter = SearchUtils.get_sql_comparison_func(
                    comparator, dialect)(attr, value)
        ...
        attribute_filters.extend(
            self._get_workspace_clauses(SqlRegisteredModel))
        ...
        if tag_filters:
            sql_tag_filters = (sqlalchemy.and_(*x)
                               for x in tag_filters.values())
            tag_filter_query = (
                select(SqlRegisteredModelTag.workspace,
                       SqlRegisteredModelTag.name)
    ...
    def get_latest_versions(self, name, stages=None):
        ...
        with self.ManagedSessionMaker() as session:
            ...
            if stages is None or len(stages) == 0:
                expected_stages = {get_canonical_stage(
            ...
            for mv in mvs:
                ...
                mv.aliases = [
                    alias.alias for alias in model_aliases if alias.version == mv.version]
    ...
    def _populate_model_version_aliases(self, session, name, version):
        model_aliases = self._get_registered_model(
            session, name).registered_model_aliases
        version.aliases = [
    ...
    def _get_sql_model_version(self, session, name, version, eager=False):
        ...
        query_options = (
            self._get_eager_model_version_query_options() if eager else []
        )
        ...
        return self._get_model_version_from_db(
            session,
            name,
            version,
            conditions,
    ...
    def _get_sql_model_version_including_deleted(self, name, version):
        ...
        with self.ManagedSessionMaker() as session:
            ...
            sql_model_version = self._get_model_version_from_db(
                session, name, version, conditions)
            return self._populate_model_version_aliases(
    ...
    def update_model_version(self, name, version, description=None):
        ...
        with self.ManagedSessionMaker(read_only=False) as session:
            ...
            sql_model_version = self._get_sql_model_version(
                session, name=name, version=version)
    ...
    def transition_model_version_stage(self, name, version, stage, archive_existing_versions):
        ...
        if archive_existing_versions and not is_active_stage:
            ...
            raise MlflowException(msg_tpl.format(
                stage, DEFAULT_STAGES_FOR_GET_LATEST_VERSIONS))
        ...
        with self.ManagedSessionMaker(read_only=False) as session:
            ...
            if archive_existing_versions:
                ...
                model_versions = self._get_query(
                    session, SqlModelVersion).filter(*conditions).all()
    ...
    def get_model_version(self, name, version):
        ...
        with self.ManagedSessionMaker() as session:
            sql_model_version = self._get_sql_model_version(
                session, name, version, eager=True)
            return self._populate_model_version_aliases(
                session, name, sql_model_version.to_mlflow_entity()
            )
    ...
    def search_model_versions(
        ...
        parsed_filters = SearchModelVersionUtils.parse_search_filter(
            filter_string)
        ...
        parsed_orderby = self._parse_search_model_versions_order_by(
            order_by or ["last_updated_timestamp DESC",
                         "name ASC", "version_number DESC"]
        )
        offset = SearchUtils.parse_start_offset_from_page_token(page_token)
        # we query for max_results + 1 items to check whether there is another page to return.
        # this remediates having to make another query which returns no items.
        ...
        with self.ManagedSessionMaker() as session:
            ...
            next_page_token = self._compute_next_token(
                max_results_for_query, len(
                    sql_model_versions), offset, max_results
            )
            model_versions = [mv.to_mlflow_entity()
                              for mv in sql_model_versions][:max_results]
    ...
    def list_webhooks(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            if has_next_page:
                ...
                next_page_token = SearchUtils.create_page_token(
                    offset + max_results)
